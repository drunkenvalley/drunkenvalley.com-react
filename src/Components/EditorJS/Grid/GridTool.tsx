import React, { ReactElement } from 'react'
import Grid from './Grid'
import { Container, EditorTool } from 'Components/EditorJS'

interface State {}

interface Props extends State {
  children?: ReactElement
}

export default class GridTool extends EditorTool<Props, State> {
  protected static _tool = {
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-grid-fill" viewBox="0 0 16 16">
        <path d="M1 2.5A1.5 1.5 0 0 1 2.5 1h3A1.5 1.5 0 0 1 7 2.5v3A1.5 1.5 0 0 1 5.5 7h-3A1.5 1.5 0 0 1 1 5.5v-3zm8 0A1.5 1.5 0 0 1 10.5 1h3A1.5 1.5 0 0 1 15 2.5v3A1.5 1.5 0 0 1 13.5 7h-3A1.5 1.5 0 0 1 9 5.5v-3zm-8 8A1.5 1.5 0 0 1 2.5 9h3A1.5 1.5 0 0 1 7 10.5v3A1.5 1.5 0 0 1 5.5 15h-3A1.5 1.5 0 0 1 1 13.5v-3zm8 0A1.5 1.5 0 0 1 10.5 9h3a1.5 1.5 0 0 1 1.5 1.5v3a1.5 1.5 0 0 1-1.5 1.5h-3A1.5 1.5 0 0 1 9 13.5v-3z"/>
      </svg>
    ),
    title: 'Grid'
  }

  private children?: ReactElement

  constructor (props: Props) {
    super(props)
    const { children, ...state } = props

    this.children = children
    this.state = state
  }

  override render = () => (
    <Container>
      <Grid className='rounded shadow' onChange={this.onChange} data={this.state}>

      </Grid>
    </Container>
  )
}
